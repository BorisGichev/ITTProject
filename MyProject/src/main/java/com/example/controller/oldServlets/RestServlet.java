...
public class RestServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Set<Activity> listWithSerachedActivities = new TreeSet<Activity>((a1, a2) -> a1.getId() - a2.getId());
			listWithSerachedActivities.addAll(IActivityDAO.getDAO("db").getAllActivitiesContainingComment(json,
					((Project) (request.getSession().getAttribute("project"))).getId()));
			response.getWriter().append(new Gson().toJson(listWithSerachedActivities));
